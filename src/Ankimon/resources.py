...
eggs_path = addon_dir / "user_files" / "eggs.json"
eggs_sprites_path = addon_dir / "user_files" / "sprites" / "eggs"
eggs_main_sprite_path = eggs_sprites_path / "MainSprite"
eggs_icon_path = eggs_sprites_path / "Icons"

# Encounter log path
encounter_log_path = addon_dir / "user_files" / "encounter_log.json"
...
def check_current_files():
    ...
    #create_folder_structure_json()  # Ensure folder structure JSON is created
    ...
    if missing_files:
        showInfo(f"The following essential files or folders are missing:\n" +
                 "\n".join(missing_files) + "\nPlease ensure they are created.")
        return False
    return True