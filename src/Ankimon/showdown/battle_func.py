...
from ..functions.battle_text_functions import effectiveness_text
...
ankimon_tracker_obj = AnkimonTracker(trainer_card=None)
translator = Translator(int(mw.settings_obj.get("misc.language")))
...
def tooltipWithColour(msg, color, x=0, y=20, xref=1, parent=None, width=0, height=0, centered=False):
    period = int(settings_obj.get("gui.reviewer_text_message_box_time") * 1000 or 3 * 1000)  # time for pop up message
...
def attack(enemy_pokemon, main_pokemon):
    ...
    if settings_obj.get("controls.allow_to_choose_moves") or True:
...
default_pokemon_data = {
    "name": "Pikachu", "id": 25, "shiny": False, "gender": "M", "level": 5, "ability": "Static",
    ...
    "attacks": ["Thunderbolt", "Quick Attack"], "base_experience": 112, "current_hp": 20, "growth_rate": "medium", "evos": ["Pikachu"], "captured_date": "2024-01-01",
    "tier": "Basic", "individual_id": "04946187465"
}
...
class PokeTeam:
            ...
            try:
                ...
                if isinstance(data, list):
                    for poke in data:
                        ...
                        main_pokemon.xp = main_pokemon.stats.get("xp", 0)
    ...
    def switch(self, pokemon, act_ind):
        ...
        if ans[1]:
            # ans[0] is the chosen name string. Protect against the
            # unlikely case where the name is missing or doesn't match
            # any `possible` entries (for example empty string). In that
            # situation pick a random valid Pokémon instead of crashing.
            try:
                chosen_name = (ans[0] or "").strip()
                names = [p.name for p in possible]
                if chosen_name and chosen_name in names:
                    new_poke = possible[names.index(chosen_name)]
                else:
                    new_poke = random.choice(possible)
            except Exception:
                new_poke = random.choice(possible)
