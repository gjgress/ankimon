...
def _process_battle_effects(
    ...
    for change in changes:
        ...
        # Track volatile status applications
            ...
            key.endswith(".volatile_status")
            and isinstance(after, set)
    ...
    def check_persistent_effects():
        ...
        if (
            current_state
            and hasattr(current_state, "weather")
            and current_state.weather
        ):
            weather_key = (
                f"weather_{normalize_status_name(current_state.weather)}_still"
            )
            weather_message = safe_translate(
                weather_key, weather=current_state.weather.replace("-", " ").title()
            )
            ...
            # Handle weather changes
                ...
                if before != after:
                    ...
                    if before is None and after is not None:
                        ...
                        message = safe_translate(
                            "battle_effect_weather_start", weather=weather_name
                        )
                        effect_messages.append(message)
                    # Weather ended
                        ...
                        message = safe_translate(
                            "battle_effect_weather_end", weather=weather_name
                        )
                        effect_messages.append(message)
                    # Weather changed
                        ...
                        message = safe_translate(
                            "battle_effect_weather_end", weather=old_weather
                        )
                        effect_messages.append(message)
                        message = safe_translate(
                            "battle_effect_weather_start", weather=new_weather
                        )
                        effect_messages.append(message)
...
def _handle_special_battle_status(main_pokemon, battle_status: str, translator) -> str:
    ...
    try:
        ...
        show_warning_with_traceback(
            exception=e, message="Error handling special battle status"
        )
        return translator.translate(
            "pokemon_special_condition",
            pokemon_name=main_pokemon.name.capitalize(),
