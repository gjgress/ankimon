...
class PokemonPC(QDialog):
    ...
    def create_gui(self):
        ...
        prev_box_button.clicked.connect(
            lambda: self.looparound_go_to_box(self.current_box_idx - 1, max_box_idx)
        )
        next_box_button.clicked.connect(
            lambda: self.looparound_go_to_box(self.current_box_idx + 1, max_box_idx)
        )
        ...
        curr_box_label.setFont(
            load_custom_font(20, int(self.settings.get("misc.language")))
        )
        curr_box_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        curr_box_label.setStyleSheet(
            f"border: 1px solid {button_border}; background-color: {background_color};"
        )
        box_selector_layout.addWidget(
            prev_box_button, alignment=Qt.AlignmentFlag.AlignCenter
        )
        box_selector_layout.addWidget(
            curr_box_label, alignment=Qt.AlignmentFlag.AlignCenter
        )
        box_selector_layout.addWidget(
            next_box_button, alignment=Qt.AlignmentFlag.AlignCenter
        )
            ...
            for col in range(self.n_cols):
                ...
                if pokemon_idx >= len(pokemon_list_slice):
                    ...
                    pokemon_grid.addWidget(
                        empty_label, col, row, alignment=Qt.AlignmentFlag.AlignCenter
                    )
                    continue
                ...
                if pokemon.get("is_favorite", False):
                    ...
                    slot_style_hover_bg = (
                        favorite_hover_color  # Favorite color doesn't change on hover
                    )
                else:
                ...
                if self.gif_in_collection:
                    ...
                    pokemon_grid.addWidget(
                        pokemon_button, col, row, alignment=Qt.AlignmentFlag.AlignCenter
                    )
                    pokemon_grid.addWidget(
                        scaled_movie_label,
                        col,
                    ...
                    pixmap = self.adjust_pixmap_size(
                        pixmap, max_width=300, max_height=230
                    )
