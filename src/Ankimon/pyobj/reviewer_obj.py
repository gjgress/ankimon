    ...
    def update_life_bar(self, reviewer, card, ease):
        ...
        if int(self.settings.get("gui.show_mainpkmn_in_reviewer")) > 0:
            if image_format == "gif":
                ...
                side = "back"
            main_pkmn_imagefile_path = self.main_pokemon.get_sprite_path(
                side, image_format
            )
        ...
        enemy_lang_name = get_pokemon_diff_lang_name(
            int(self.enemy_pokemon.id), int(self.settings.get("misc.language"))
        ).capitalize()
        ...
        if int(self.settings.get("gui.show_mainpkmn_in_reviewer")) > 0:
            ...
            main_lang_name = get_pokemon_diff_lang_name(
                int(self.main_pokemon.id), int(self.settings.get("misc.language"))
            ).capitalize()
