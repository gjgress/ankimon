...
class PokemonCollectionDialog(QDialog):
    def __init__(
        ...
        self.pagination_layout = QHBoxLayout(
            self.paginator
        )  # generate paginationlayout to delete later
        ...
        try:
            ...
            for pokemon in paginated_pokemon:
                ...
                name_label = self.create_label(
                    pokemon_nickname
                    or f"{pokemon_name.capitalize()} {self.get_gender_symbol(pokemon_gender)}",
                    12,
                )
                level_label = self.create_label(f"Level: {pokemon_level}", 8)
                type_label = self.create_label(
                    "Type: " + " ".join([t.capitalize() for t in pokemon_type]), 8
                )
                ...
                pokemon_button = self.create_button(
                    "Show me Details", pokemon, "Show Details"
                )
                choose_pokemon_button = self.create_button(
                    "Pick as main Pokemon", pokemon, "Pick As Main"
                )
    ...
    def create_button(self, text, pokemon_data, button_type):
        ...
        if button_type == "Show Details":
            button.clicked.connect(
                lambda state: self.show_pokemon_details(pokemon_data)
            )
            ...
            button.clicked.connect(
                lambda state: self.main_pokemon_function_callback(pokemon_data)
            )
...
def PokemonTrade(name, id, level, ability, iv, ev, gender, attacks, position):
    ...
    for pokemons in pokemon_data:
        if (
            pokemons["name"] == name
            and pokemons["id"] == id
            and pokemons["level"] == level
            and pokemons["ability"] == ability
            and pokemons["iv"] == iv
            and pokemons["ev"] == ev
            and pokemons["gender"] == gender
            and pokemons["attacks"] == attacks
        ):
            found = True
            break
    ...
    if not found:
        ...
        qconnect(
            ...
            lambda: PokemonTradeIn(trade_code_input.text(), name, position),
        )
        # Information label
...
def PokemonTradeIn(number_code, old_pokemon_name, position):
    if len(number_code) > 15:
        ...
        logger.log_and_showinfo(
            "info", f"You have sucessfully traded your {old_pokemon_name} for {name} "
        )
    else:
